...
if direction_of_stream_towards_boat=="along":
    ...
    if distance=="?":
        ...
        print(formula)
        speed_of_boat=int(speed_of_boat)
        speed_of_stream=int(speed_of_stream)
        time=int(time)
        print("i.e Distance=(Speed of boat+speed of stream)*(Time)")
        total_distance=round((speed_of_boat+speed_of_stream)*(time),2)
        if total_distance>=0:
            print(" ")
            print(Fore.GREEN+"Distance : "+str(total_distance)+" km")
        else:
            print(Fore.RED+"Invalid detalis")
    elif time=="?":
        formula="Time = Distance/(speed of boat+speed of stream)"
        print("if boat goes along the direction of stream : ")
        print(formula)
        ...
        total_time=round(distance/(speed_of_boat+speed_of_stream),2)
        if total_time>=0:
            print(" ")
        ...
        total_speed_of_boat=round((distance/time)-speed_of_stream,2)
        ...
        total_speed_of_stream=round((distance/time)-speed_of_boat,2)
...
if direction_of_stream_towards_boat=="along and against" or direction_of_stream_towards_boat=="against and along":
    ...
    downstream_speed=round(float(downstream_distance)/float(downstream_time),2)
    print("Downstream speed : "+str(downstream_speed)+" km/hr")
    print("upstream speed=upstream distance/upstream time")
    upstream_speed=round(float(upstream_distance)/float(upstream_time),2)
    ...
    if speed=="speed of boat":
        ...
        speed_of_boat=round((int(downstream_speed)+int(upstream_speed))/2,2)
        if speed_of_boat>=0:
        ...
        speed_of_stream = round((int(downstream_speed) - int(upstream_speed)) / 2,2)
